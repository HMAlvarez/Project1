	...
	public Story add(Story s) {
		...
		try {
			...
			if (s.getAssistant() == null)
				...
				ps.setInt(11, s.getAssistant().getId());
			if (s.getEditor() == null)
				ps.setNull(12, java.sql.Types.INTEGER);
			else
				ps.setInt(12, s.getEditor().getId());
			if (s.getSenior() == null)
				ps.setNull(13, java.sql.Types.INTEGER);
			else
				ps.setInt(13, s.getSenior().getId());
	...
	public List<Story> getAllByReceiverName(String firstName, String lastName) {
		...
		try {
			...
			while (rs.next()) {
				...
				if (s.getResponse() == null || s.getResponse().equals(""))
					list.add(s);
			}
	...
	public boolean update(Story s) {
		...
		try {
			...
			if (s.getAssistant() == null)
				...
				ps.setInt(7, s.getAssistant().getId());
			if (s.getEditor() == null)
				ps.setNull(8, java.sql.Types.INTEGER);
			else
				ps.setInt(8, s.getEditor().getId());
			if (s.getSenior() == null)
				ps.setNull(9, java.sql.Types.INTEGER);
			else
				ps.setInt(9, s.getSenior().getId());
