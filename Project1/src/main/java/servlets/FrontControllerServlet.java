...
import services.EditorServices;
import services.GEJoinServices;
import services.GenreServices;
import services.StoryServices;
import services.StoryTypeServices;
...
public class FrontControllerServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// TODO: can editor login and author login be combined into the same thing?
		// would require that login info across the two tables be unique
		case "author_login": {
		...
		case "get_proposals": {
			...
			if (logged_in instanceof Author) {
				...
				for (Genre g : genres) {
					if (e.getSenior()) {
						stories.addAll(
								StoryServices.getInstance().getAllByGenreAndStatus(g.getId(), "approved_editor"));
