...
//	}
	...
	public void submitNextWaitingProposal(Author a) {
		...
		Story next = waiting.stream().filter((s) -> {
			return s.getType().getPoints() <= a.getPoints();
		}).findAny().orElse(null);
		if (next != null) {
	...
	public List<Story> getAllWithDraftsForEditor(Editor e) {
		// TODO: refactor this method so that most of the logic is in here instead of in
		// the repo,
